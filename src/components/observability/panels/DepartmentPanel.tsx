...
import { getTopConsumers, getDepartmentObservability } from '../../../data/observabilityApi';
import { Drill } from '../../views/ObservabilityView';

export const DepartmentPanel: React.FC<{ onDrill: (d: Drill) => void }> = ({ onDrill }) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <article className="platform-card p-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9.5px] font-bold uppercase tracking-wider text-slate-400">Active Departments</span>
          </div>
      ...
      <section className="platform-card p-5">
        ...
        <p className="mt-0.5 text-[11px] text-slate-500">Departments ranked by LLM cost — chargeback basis.</p>
        <div className="mt-4 space-y-2.5">
          {consumers.map((c, i) => {
            const departmentObs = getDepartmentObservability(c.department_id);
            return (
              <button
                key={c.department_id}
                onClick={() => onDrill({ level: 'L3', departmentId: c.department_id })}
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between">
                    <span className="text-[12px] font-bold text-slate-800">
                      {c.department_schema.replace('department_', '').replace(/_/g, ' ')}
                    </span>
                  ...
                  <div className="mt-1.5 flex gap-4 text-[10px] text-slate-400">
                    ...
                    <span>{departmentObs.active_member_count} members</span>
                    <span>{departmentObs.project_count} projects</span>
                  </div>
                </div>
      ...
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {consumers.map((c) => {
          const obs = getDepartmentObservability(c.department_id);
          const failureRate = obs.recent_run_count === 0 ? 0 : (obs.recent_run_failures / obs.recent_run_count) * 100;
          return (
            <article key={c.department_id} className="platform-card p-4">
              <div className="flex items-center justify-between">
